    ...
    *,
    ...
    for row in range(1, max_row + 1):
        ...
        for col in range(1, max_col + 1):
            ...
            lines.append(f"    {_render_cell(doc_ir, cell, debug_layout=debug_layout)}")
        lines.append("  </tr>")
    lines.append("</table>")
    return "\n".join(lines)
...
def _column_group_debug_label(style: ParaStyleInfo, paragraphs: list[ParagraphIR]) -> str:
    debug_paths = ", ".join(_node_debug_path(paragraph) for paragraph in paragraphs)
    return f"columns x{style.column_count or 1}: gap {_pt_label(_non_negative_pt(style.column_gap_pt))}; {debug_paths}"
